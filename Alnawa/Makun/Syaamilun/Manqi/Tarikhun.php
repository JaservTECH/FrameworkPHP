...
/**
 * Tarikhun berarti Date atau tanggal
 * Class yang meng-handle data berbentuk datetime
 */
	...
	*/
	...
	public function getDate($FORMAT = NULL,$default=true){
		if($default){
			...
			$FORMAT = str_ireplace("o",$this->nice_date($this->baseDate,"m"),$FORMAT);
			$FORMAT = str_ireplace("L",$this->nice_date($this->baseDate,"d"),$FORMAT);
	...
	public function isBefore($TEMP_DATE,$NEXT=false,$FORCE_NOW=false){
		...
		$tempDateInt = intval($this->human_to_unix($TEMP_DATE));
		$tempBaseInt = intval($this->human_to_unix($this->baseDate));
		if($tempBaseInt < $tempDateInt){
			return $this->returnIs($NEXT,true);
		}else if($tempBaseInt == $tempDateInt){
			if($FORCE_NOW)
				return $this->returnIs($NEXT,true);
			else
				return $this->returnIs($NEXT,false);
